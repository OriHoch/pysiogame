...
class Board(gd.BoardGame):
    ...
    def create_game_objects(self, level = 1):
        ...
        img_top = 2
        img_size = 9
        base_len = (img_left - 1) * 2 + img_size
        self.words = ["caterpillar", "hippopotamus"]
        img_src = "caterpillar.jpg"
        #frame around image
        ...
        self.board.units[-1].set_outline(color = [200,0,0], width = 1)
        ...
        for i in range(len(self.words[0])):
            self.board.add_unit(2+i,16,1,1,classes.board.Letter,self.words[0][i],white,"",0)
            self.board.ships[-1].highlight = False
            self.board.ships[-1].outline_highlight = True
            self.board.ships[-1].set_outline(color = [200,200,200], width = 1)
