...
class PEdit(pygame.sprite.Sprite):
    def __init__(self,loginscreen, w,h,l,t,focus_order,hide = False, right_align = False, transparent = False):
        ...
        if self.update_me and self.visible:
            ...
            if self.right_align:
                ...
                if isinstance(self, PButton):
                    self.font_x = (self.w - self.loginscreen.font_2.size(val)[0])//2
                else:
                    self.font_x = 5
    ...
    def onMouseButtonDown(self):
        self.onFocus()
        if self.select_item and self.focus_order > -1 and self.loginscreen.state == "LOGIN":
            if self.value != "":
                self.onBlur()
                self.loginscreen.username.value = self.value
                self.loginscreen.reload_selects()
                if self.loginscreen.require_pass:
                    if ex.unival(self.loginscreen.username.value) != ex.unival(self.loginscreen.lang.b["Guest"]):
                        self.loginscreen.in_focus = self.loginscreen.password
                        self.loginscreen.password.onFocus()
                    else:
                        #self.loginscreen.in_focus = self.loginscreen.loginbtn
                        #self.loginscreen.update_me = True
                        #self.loginscreen.loginbtn.onFocus()
                        self.loginscreen.flogin()
                else:
                    #self.loginscreen.in_focus = self.loginscreen.username
                    #self.loginscreen.username.onFocus()
                    self.loginscreen.flogin()
                
                self.loginscreen.set_scrollbar_top(self.loginscreen.scroll_min_top)
        if self.users and self.focus_order > -1 and self.loginscreen.state == "USERS":
            self.loginscreen.fdetails(self.value) #print("clicked on user")
    ...
    def nextFocus(self):
        self.loginscreen.nextFocus(self.focus_order)
        
    def update_trigger(self):
        self.update_me = True
        self.loginscreen.update_me = True
        self.loginscreen.mainloop.redraw_needed[0] = True

class PCheckbox(pygame.sprite.Sprite):
    def __init__(self,loginscreen, w,h,l,t,checked,value,hide = False):
        pygame.sprite.Sprite.__init__(self)
        self.loginscreen = loginscreen
        self.w = w
        self.h = h
...
class PLabel(pygame.sprite.Sprite):
    def __init__(self,loginscreen, w,h,l,t,value):
        pygame.sprite.Sprite.__init__(self)
        self.loginscreen = loginscreen
        self.w = w
        self.h = h
        ...
        self.rect = self.image.get_rect()
        self.rect.topleft = [l,t]
        self.font_v = self.loginscreen.font_2
...
class PButton(PEdit):
    def __init__(self,loginscreen, w,h,l,t,focus_order,value,fsubmit, right_align = False, transparent = False):
        PEdit.__init__(self,loginscreen, w,h,l,t,focus_order, False, right_align, transparent)
        self.value = value
        self.fsubmit = fsubmit
        if not self.transparent:
            self.bg_color = (205,255,155)
            self.bg_focus = (105,250,100)
            self.font_color = (0, 0, 0)
            
            self.border_color = (0, 0, 0)
            self.border_focused = (0, 255, 0)
        else:
            self.bg_color = (55,55,55)
            self.bg_focus = (55,55,55)
            self.font_color = (255, 85, 85)
            
            self.border_color = (55,55,55)
            self.border_focused = (55,55,55)
...
class PScrollBar(pygame.sprite.Sprite):
    def __init__(self,loginscreen, w,h,l,t,focus_order,hide = False):
        pygame.sprite.Sprite.__init__(self)
        self.loginscreen = loginscreen
        self.w = w
        self.h = h
    ...
    def update(self):
        if self.update_me:
            #self.update_me = False
            if self.focused:
                if self.loginscreen.usr_count > 5:
                    self.image.fill((255,250,200))
            ...
            for i in range(3):
                pygame.draw.line(self.image, (170,170,170), s[i], e[i],1)
    ...
    def onMouseButtonDown(self, event):
        self.onFocus()
        #self.loginscreen.username.value = self.value
        self.loginscreen.scroll_down = True
...
class PIMGButton(pygame.sprite.Sprite):
    def __init__(self,loginscreen, w,h,l,t,focus_order,img_src,img_src2,fsubmit):
        pygame.sprite.Sprite.__init__(self)
        self.loginscreen = loginscreen
        self.w = w
        self.h = h
...
class LoginScreen:
    def __init__(self, mainloop, screen, size):
        ...
        self.font_color = (255, 255, 255)
        self.header_font_color = (205,255,155)
        self.bg_col = (55,55,55)
        self.bg_sidecol = (40,40,40)
        ...
        # headers
        self.font_1 = pygame.font.Font(os.path.join('res', 'fonts', 'FreeSansBold', 'FreeSansBold.ttf'), (int(self.points*2.0)))
        
        # buttons & edit fields
        self.font_2 = pygame.font.Font(os.path.join('res', 'fonts', 'FreeSansBold', 'FreeSansBold.ttf'), (int(self.points*1.8)))
        
        # labels
        self.font_3 = pygame.font.Font(os.path.join('res', 'fonts', 'FreeSansBold', 'FreeSansBold.ttf'), (int(self.points*1.8)))
    ...
    def merge_sprite_lists(self):
        ...
        self.db_status_lbl.bg_color = (70,70,70)
    ...
    def add_lang_elements(self):
        ...
        for i in range(self.scroll_item_count):
            ...
            self.select[i].bg_color = (45,45,45)
            self.select[i].bg_focus = (55,55,55)
            self.select[i].border_color = (30, 30, 30)
    ...
    def add_users_elements(self):
        ...
        for i in range(self.scroll_item_count):
            ...
            self.select[i].users = True
            self.select[i].bg_color = (45,45,45)
            self.select[i].bg_focus = (55,55,55)#(85,85,85)
            self.select[i].border_color = (30, 30, 30)
            self.select[i].border_focused = (150, 0, 0)
            self.select[i].font_color = (255, 255, 255)
        ...
        form_elements = [self.lb1, self.lb2, self.lb3, self.lb4, self.rusername, self.rpassword, self.rconfirmpassword, self.rregisterbtn, self.ilb1, self.ilb2, self.ilb3, self.dlb1, self.dlb2, self.dlb3, self.dlb4,self.delete_yes,self.delete_no, self.delete_btn, self.delete_imgbtn]
    ...
    def add_prefs_elements(self):
        ...
        self.cb2 = PCheckbox(self,665,30,self.left+20,self.top+120,False, self.lang.b["display languages with uncompleted translations"])
        self.edit_list.add(self.cb2)
        
        self.cb3 = PCheckbox(self,665,30,self.left+20,self.top+150,False, self.lang.b["require password to log in"])
        self.edit_list.add(self.cb3)
        
        self.cb4 = PCheckbox(self,665,30,self.left+20,self.top+180,False, self.lang.b["require password to access admin area"])
